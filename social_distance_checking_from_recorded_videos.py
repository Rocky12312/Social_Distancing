...
#Path to Yolo weights and model configuration
...
#Determinimg the output layer names that are going to need from Yolo('yolo_82', 'yolo_94', 'yolo_106'--these are the layers)
layernames_needed = net.getLayerNames()
layernames_needed = [layernames_needed[i[0] - 1] for i in net.getUnconnectedOutLayers()]


#Capturing the frames if want to use some video files for the task just provide the video file path at place of 0 in next line(0 for capturing the live streams).
cap = cv2.VideoCapture("road_crowd.mp4")
